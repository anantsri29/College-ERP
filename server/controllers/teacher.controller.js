...
exports.getDashboard = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        data: { subjects, stats: { assignmentCount, pendingGrading } },
      });
  } catch (err) {
    next(err);
  }
...
exports.getSubjectStudents = async (req, res, next) => {
  try {
    ...
    if (!subject)
      return next(
        new AppError("Subject not found or not assigned to you.", 404),
      );
...
exports.markAttendance = async (req, res, next) => {
  try {
    ...
    if (!subjectDoc)
      return next(new AppError("Subject not assigned to you.", 403));
    ...
    if (existing)
      return next(
        new AppError("Attendance already marked for this date.", 400),
      );
...
exports.updateAttendance = async (req, res, next) => {
  try {
    ...
    if (attendance.isLocked)
      return next(new AppError("Attendance is locked.", 403));
...
exports.gradeSubmission = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Submission graded.", data: assignment });
  } catch (err) {
    next(err);
  }
};
...
exports.createOrUpdateResult = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Result saved.", data: result });
  } catch (err) {
    next(err);
  }
};
...
exports.bulkCreateResults = async (req, res, next) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: `${saved.length} results saved.`,
        data: saved,
      });
  } catch (err) {
    next(err);
...
exports.publishResults = async (req, res, next) => {
  try {
    ...
    await Result.updateMany(
      { subject, semester, academicYear, enteredBy: req.user._id },
...
exports.sendAnnouncement = async (req, res, next) => {
  try {
    ...
    // Emit socket notification to all recipient students
    const io = req.app.get("io");
    const notificationData = notification.toObject();
    students.forEach((student) => {
      io.emitToUser(student._id, "notification:new", notificationData);
    });
